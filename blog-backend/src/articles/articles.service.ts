...
export class ArticlesService {
  ...
  async searchArticles(opts: { q: string; page: number; limit: number; category?: string }) {
    ...
    // Deep match: FTS vector + substring match on title/excerpt/content + tags + categories.
    const deepMatch = Prisma.sql`(
      (
        search_vector IS NOT NULL
        AND plainto_tsquery('simple', unaccent(${query})) <> ''::tsquery
        AND search_vector @@ plainto_tsquery('simple', unaccent(${query}))
      )
      OR unaccent(coalesce(title->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR unaccent(coalesce(title->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR unaccent(coalesce(excerpt->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR unaccent(coalesce(excerpt->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR unaccent(coalesce(content->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR unaccent(coalesce(content->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
      OR EXISTS (
        SELECT 1
        ...
        WHERE ac.article_id = articles.id
          AND (
            unaccent(coalesce(c.name->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(c.name->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(c.slug->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(c.slug->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
          )
      )
      OR EXISTS (
        SELECT 1
        ...
        WHERE at.article_id = articles.id
          AND (
            unaccent(coalesce(t.name->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(t.name->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(t.slug->>'en', '')) ILIKE unaccent(${like}) ESCAPE '\\'
            OR unaccent(coalesce(t.slug->>'ru', '')) ILIKE unaccent(${like}) ESCAPE '\\'
          )
      )
    )`;
      ...
      SELECT
        ...
        CASE
          WHEN search_vector IS NOT NULL
            AND plainto_tsquery('simple', unaccent(${query})) <> ''::tsquery
            AND search_vector @@ plainto_tsquery('simple', unaccent(${query}))
          THEN ts_rank(search_vector, plainto_tsquery('simple', unaccent(${query})))
          ELSE 0.05
        END AS rank
      FROM articles
      WHERE deleted_at IS NULL
    ...
    return {
      items: articles.map((a) => this.mapArticle(a as unknown as Record<string, unknown>)),
      pagination: {
        page: opts.page,
        limit: opts.limit,
        total,
